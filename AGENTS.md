...
- **Themes (2026-03-20)**:
  - Добавлен переключаемый из header альтернативный dark theme, вдохновленный Quanta AI branding: неоновые cyan/indigo акценты, тёмные glass-surfaces и адаптированные токены интерфейса.
  - Переключение темы работает на уровне глобальных CSS-токенов через `data-theme`, поэтому существующие компоненты автоматически наследуют новую палитру без локальных hex-override.
